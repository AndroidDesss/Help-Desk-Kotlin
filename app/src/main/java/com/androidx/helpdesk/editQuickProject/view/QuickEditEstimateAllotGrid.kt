...
import com.androidx.helpdesk.databinding.ActivityQuickEditEstimateAllotGridBinding
import com.androidx.helpdesk.editQuickProject.adapter.EditGridListAdapter
import com.androidx.helpdesk.editQuickProject.model.EditGridListModel
...
class QuickEditEstimateAllotGrid : AppCompatActivity(), OnProgrammerListener {

    private var binding: ActivityQuickEditEstimateAllotGridBinding? = null
    ...
    private var editGridList: MutableList<EditGridListModel> = ArrayList()

    private var editGridListAdapter: EditGridListAdapter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_quick_edit_estimate_allot_grid)
        getBundleData()
        initListener()
    }
                    ...
                    if (status == 200) {
                        ...
                        if (dataArray != null && dataArray.length() > 0) {
                            for (i in 0 until dataArray.length()) {
                                ...
                                editGridList.add(EditGridListModel(gridProjectTaskAllotId,gridProjectId,gridModuleId,gridProjectTaskId,gridStartDate,gridEndDate,gridTaskDate,gridTaskStatus,gridTaskName,gridSequence,gridAssignedBy,gridAssignedTo,gridTaskType,gridTaskHours))
                            }
                            binding!!.btnContainer.visibility = View.VISIBLE
                            editGridListAdapter = EditGridListAdapter(this, editGridList,this)
                            binding!!.recyclerView.adapter = editGridListAdapter
                            editGridListAdapter!!.notifyDataSetChanged()
                            editGridListAdapter!!.setOnClickListener(object :
                                EditGridListAdapter.OnClickListener {
    ...
    override fun onProgrammerItemClicked(position: Int) {
        if (newAllottedList.contains(editGridList[position].gridProjectTaskAllotId))
        {
            newAllottedList.remove(editGridList[position].gridProjectTaskAllotId)
            newModuleList.remove(editGridList[position].gridModuleId)
            newSubModuleList.remove(editGridList[position].gridProjectTaskId)
        }
        else
        {
            newAllottedList.add(editGridList[position].gridProjectTaskAllotId)
            newModuleList.add(editGridList[position].gridModuleId)
            newSubModuleList.add(editGridList[position].gridProjectTaskId)
            newProjectId = editGridList[position].gridProjectId
        }
    }
