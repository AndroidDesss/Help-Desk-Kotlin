...
class EditGridListAdapter(private val context: Context?, private val gridList: List<EditGridListModel>, private val OnProgrammerClickListener: OnProgrammerListener) : RecyclerView.Adapter<EditGridListAdapter.ConnectionsHolder>() {
    var gridListModel: EditGridListModel? = null
    private var  onClickListenerGridList: OnClickListener?= null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EditGridListAdapter.ConnectionsHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.adapter_grid_list, parent, false)
        return ConnectionsHolder(v)
    }

    override fun onBindViewHolder(holder: EditGridListAdapter.ConnectionsHolder, position: Int) {
