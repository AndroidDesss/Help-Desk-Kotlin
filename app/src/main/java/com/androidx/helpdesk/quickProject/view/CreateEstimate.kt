...
import com.androidx.helpdesk.databinding.ActivityCreateEstimateBinding
import com.androidx.helpdesk.quickProject.`interface`.OnProgrammerListener
import com.androidx.helpdesk.quickProject.adapter.TaskListAdapter
...
class CreateEstimate : AppCompatActivity(), OnProgrammerListener {

    private var binding: ActivityCreateEstimateBinding ? = null
    ...
    private var taskList: MutableList<TaskListModel> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_create_estimate)
        getBundleData()
        initListener()
    ...
    private fun getScreenNameList(moduleId: Int)
    {
        ...
        stringRequest = StringRequest(Request.Method.POST, Api.getEstimateScreenName + SharedPref.getCompanyId(this) + "&ModuleID=" + moduleId,
            { ServerResponse ->
                try {
                    val jsondata = JSONObject(ServerResponse)
                    status = jsondata.getInt("status")
                ...
                try {
                    ...
                    if (status == 200) {
                        ...
                        if (dataArray != null && dataArray.length() > 0) {
                            ...
                            taskListAdapter = TaskListAdapter(this, taskList,this)
    ...
    override fun onProgrammerItemClicked(position: Int) {
        ...
        else
        {
            ...
            taskCategorySpinnerId = taskList[position].taskResponseCategoryId
        }
    }
                    ...
                    if (status == 200) {
                        ...
                        if (dataArray != null && dataArray.length() > 0) {
                            ...
                            if (errMsg.equals("Inserted Successfully") || errMsg == "Inserted Successfully") {
                                ...
                                startActivity(Intent(this, CreateEstimateAllotGrid::class.java).putExtra("projectId",projectId).putExtra("projectName",projectName).putExtra("moduleId",mId).putExtra("subModuleId",smId).putExtra("taskCategoryId",taskCategoryId))
                            }
                        }
                    }
                } catch (e: JSONException) {
